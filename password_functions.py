...
def add_note():
    ...
    # add note to password
    c.execute("UPDATE passwords SET notes = ? WHERE username = ?", (note, username))
    conn.commit()
    c.close()
    conn.close()
...
def random_passwords():
    ...
    for i in range(length):
        ...
        random_user = "".join(
            random.choice(s.ascii_letters) for i in range(random.randint(5, 10))
        )
...
def find_password():
    ...
    c.execute(
        "SELECT * FROM passwords WHERE username LIKE ? OR notes LIKE ?",
        ("%" + name + "%", "%" + name + "%"),
    )
...
def link_account():
    ...
    # create table called passwords if it does not exist
    create_database()
    c.execute("INSERT INTO passwords VALUES (?, ?, ?)", (user, pas, "Empty note"))
    conn.commit()
    c.close()
    conn.close()
